...
status SaveResult(int result, double time, double time_, bool value[],
                  char fileName[], int boolCount); // 保存求解结果
void Sudoku();                                     // X数独
...
status Generate_Sudoku(int board[SIZE + 1][SIZE + 1],
                       ...
                       bool value[SIZE * SIZE * SIZE + 1]); // 生成数独
status Is_Valid(int board[SIZE + 1][SIZE + 1], int row, int col,
                int v); // 判断board[row][col]是否可以填入
void Print_Sudoku(int board[SIZE + 1][SIZE + 1]); // 打印数独
void Play_Sudoku(int board[SIZE + 1][SIZE + 1],
                 bool isFixed[SIZE + 1][SIZE + 1]); // 玩数独
status WriteToFile(int board[SIZE + 1][SIZE + 1], int num,
                   char name[]); // 将数独约束条件写入文件
status Slove(int board[SIZE + 1][SIZE + 1],
             bool value[SIZE * SIZE * SIZE + 1]); // 求解数独
status Fill_Box(int board[SIZE + 1][SIZE + 1], int newBoard[SIZE + 1][SIZE + 1],
                int newBoard2[SIZE + 1][SIZE + 1], int rowStart,
                int colStart);  // 填充3x3的宫格
void Shuffle(int arr[], int n); // 打乱数组
status Validate_Sudoku(
    int board[SIZE + 1][SIZE + 1]); // 用于验证数独是否满足所有约束条件
status
HasUniqueSolution(int board[SIZE + 1][SIZE + 1],
                  bool isFixed[SIZE + 1][SIZE + 1],
                  int solution[SIZE + 1][SIZE + 1]); // 用于检查数独是否有唯一解